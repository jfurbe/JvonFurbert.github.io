                   ...
                   {row.cells.map(cell => {
                     return (
                       <td
                         {...cell.getCellProps()}>
                         {/*cell.render('Cell')*/}
                         {(['PAYMENTS','RECEIPTS','ASSETS'].includes(cell.column.Header)) ? ReactHtmlParser(cell.value) : cell.render('Cell')}
...
const cols = createCols(dataIn[0], nonEdit);
 const columns = React.useMemo(
  ...
  setData(old =>
    old.map((row, index) => {
      if (index === rowIndex) {
        if (Object.keys(currentEAS.dic3).includes(data[index].item)){
          ...
          if (columnId==='Dec2019'){
            ...
            currentEAS.dic4[data[index].item][1] = value
          }
        }
